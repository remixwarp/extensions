...
(function (Scratch) {
  ...
  // 优化1：强化Feature Policy，禁用所有可能触发外部跳转/弹窗的能力
  const featurePolicy = {
    accelerometer: "'none'",
    ...
    "navigation-override": "'none'", // 新增：禁用导航覆盖
    "popups": "'none'", // 新增：禁用弹窗
  };

  // 优化2：强化沙箱策略，明确禁用顶部导航（防止跳转到外部浏览器）
  const SANDBOX = [
    "allow-same-origin",
    "allow-scripts",
    "allow-forms",
    "allow-modals",
    "allow-popups-to-escape-sandbox", // 仅允许沙箱内弹窗，不跳外部
    "allow-downloads", // 可选：允许沙箱内下载（如需禁用可删除）
    "allow-pointer-lock", // 可选：允许指针锁定（如需禁用可删除）
    // 核心：禁用 allow-top-navigation（防止iframe跳转到顶层窗口/外部浏览器）
    // 核心：禁用 allow-top-navigation-by-user-activation（防止用户触发的外部跳转）
  ];
  ...
  // 优化3：创建iframe时添加导航拦截，防止跳转到外部浏览器
  const createFrame = (src) => {
    closeFrame(); // 先关闭旧iframe，避免多实例
    ...
    iframe.style.zIndex = "9999"; // 确保iframe在顶层展示
        ...
        interceptScript.textContent = `
          ...
          document.addEventListener('click', (e) => {
            ...
            if (target && target.href) {
              ...
              // 可选：在iframe内加载新URL，而非跳外部
              window.location.href = target.href;
            }
          });
          // 拦截window.open（防止通过JS打开外部浏览器）
          const originalOpen = window.open;
          window.open = (url) => {
            // 在当前iframe内打开URL，而非新窗口
          ...
          Object.defineProperty(window, 'location', {
            set: (newVal) => {
              // 仅允许在iframe内跳转，不触发外部导航
              if (typeof newVal === 'string') {
                history.pushState({}, '', newVal);
                originalLocation.set.call(window, newVal);
              }
            },
            get: () => originalLocation.get.call(window)
          });
        `;
        iframe.contentDocument.head.appendChild(interceptScript);
      } catch (e) {
        // 跨域时注入脚本会失败，属于正常情况
        console.warn("导航拦截脚本注入失败（跨域限制）:", e);
      }
    });

    // 拦截iframe的beforeunload事件，防止意外跳转
    iframe.addEventListener("beforeunload", (e) => {
      e.preventDefault();
      e.returnValue = ""; // 阻止默认卸载行为
    });
  ...
  class IframeExtension {
    ...
    async display({ URL }) {
      closeFrame();
      // 优化4：验证URL合法性，仅允许http/https（避免file://等协议跳转）
      const normalizedURL = Scratch.Cast.toString(URL).trim();
      if (!/^https?:\/\//i.test(normalizedURL)) {
        console.warn("仅支持HTTP/HTTPS协议的URL:", normalizedURL);
        return;
      }
      if (await Scratch.canEmbed(normalizedURL)) {
        createFrame(normalizedURL);
      }
    }
    ...
    show() {
      if (iframe) {
        iframe.style.display = "block"; // 明确设为block，避免隐藏
      }
    }
